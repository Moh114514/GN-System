...
| 2026-08-14，当前 `develop` | 工作区未发布 | PR1–PR5 已加入：客户/订单归属收敛、客户状态树、提醒 UI 紧凑化、指定节假日客服提醒及 Dashboard 数据下钻 | PR1 含不可逆 migration；PR2–PR5 不新增 migration。发布前按完整门禁执行，备份并核对 PR1 数据前置条件；UAT/Production 需人工验收状态树、提醒页面、指定日期规则及 Dashboard 日期范围跳转和超级管理员入口 |
| 2026-08-14，当前 `feature/settlement-generation-schedule` | 工作区未发布 | PR6 将月结统计周期固定为自然月，并在每月 10 日指定时间生成上一个自然月；保留旧配置边界重建、停机补偿和同周期幂等 | 新增 `2026_08_14_000200_add_generation_day_to_settlement_configurations`；发布前备份并核对既有配置、历史 Run 和快照，UAT 需按 9 日/10 日/11 日/次月 10 日矩阵验收；本机结果不能替代目标环境验证 |
