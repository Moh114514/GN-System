...
| 2026-08-07，当前 `develop` | 工作区未发布 | 国际化 PR-A 基础设施、PR-B 日常界面及 PR-C 第一批页面：配置中心/内部用户、结算、提醒、多维查询和导入支持 `zh_CN`/`ko_KR` | 含 `2026_08_07_000100_add_preferred_locale_to_users_table` migration；导出文件名、队列/通知任务、深层异常码和动态错误正文仍待完成，发布前备份数据库并核对既有用户均为 `zh_CN`，韩文入口暂未开放 |

当前 `main` 高于 `v0.5.0-rc.8`。服务器上的 `releases/current` 和
`history.tsv` 才能证明 UAT/Production 实际运行版本；本地 Git 日志不能证明目标环境已经升级。

国际化 PR-A、PR-B 及 PR-C 第一批当前只在 `develop` 工作区完成，尚未发布到 UAT 或 Production。发布该变更时，
