...
| `develop`（2026-08-14 工作区） | 国际化基础设施、PR-B/PR-C/PR-D 及规划 PR1–PR5 已加入；PR4 增加指定节假日客服提醒，PR5 增加 Dashboard 数据下钻 | 尚未发布；PR1 包含不可逆 `2026_08_14_000100_remove_direct_sales_business` migration，PR2–PR5 不新增 migration。发布前备份数据库，核对 PR1 数据前置条件，并人工检查客户状态树、提醒文案、指定日期规则和 Dashboard 日期范围跳转/权限入口 |
| `feature/settlement-generation-schedule`（2026-08-14 工作区） | PR6 将月结改为自然月统计、每月 10 日生成上月，并保留旧边界历史重建和 Scheduler 补偿 | 尚未发布；新增 `2026_08_14_000200_add_generation_day_to_settlement_configurations`。发布前备份数据库，核对既有配置/历史 Run/快照，并在 UAT 按 9 日、10 日 08:59/09:00、11 日和次月 10 日验收 |
...
“不适用”的历史月结只能查看，不能重新生成。月结生成后如果补录了同周期订单，待审核或已驳回月结会显示“源数据已发生变化”；管理员填写刷新原因后可以刷新明细，系统会同时更新原批次金额并记录审计。已通过、已结清、历史已支付、历史已对账月结不能直接刷新。未关联批次的历史月结请从“历史月结归档”入口查询，历史明细使用月结快照，已删除订单会显示为已归档订单。
