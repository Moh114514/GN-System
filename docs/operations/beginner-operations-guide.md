...
| `feature/pr1-order-registration`（2026-09-14 本地工作区） | 方案 PR4 客户工作台迁移；PR5 增加按机构月度正式口径的当前月营收对比、客户移交 BD 站内/钉钉通知，以及 Excel 回传凭证强制校验 | 不新增 migration、依赖或环境变量；仅完成本地自动化验证，未合入 `develop`、未创建 RC 或部署；UAT 要检查机构范围、当月零营收活动机构、当前月销售下钻、BD/客服通知对象、审批人不自收钉钉和 Excel 两类凭证拒绝路径 |
...
`php artisan migrate --force` → 执行 `php artisan optimize:clear` → 启动应用 → 重启 Queue Worker → 检查健康接口。
在 migration 和缓存清理完成前不要打开业务页面；否则代码可能已经是新版本，数据库仍是旧结构，
会出现页面访问数据库列不存在的错误。

开发环境的 Queue 是 `queue:listen`，修改 Provider 后下一次任务会重新加载代码；UAT 使用
`queue:work`，部署脚本会执行 `queue:restart`，也可以在确认目标是 UAT 后手动重启 Queue 容器。
...
而是使用业务日期起止；只要月结周期与查询区间相交就应命中。已结清月结重新打开详情后，审核时生成的 Word/PDF
仍应保留并可下载；历史 `paid`/`reconciled` 月结可在只读详情点击生成文档后下载，且状态不变。
...
主要业务异常码、已知审计动作、历史固定审计描述、默认客户状态、系统提醒模板和安全失败文案的韩文化已完成；未知历史自由文本会保留原文或显示安全通用说明，已知动态错误保存消息键与命名参数。语言设置页可直接选择 `한국어`；发布到 UAT/Production 前仍需按 RC 流程执行至 `000500` 的 migration，并抽查韩文配置页、客户状态、系统提醒、月结失败和最近导出。回退 migration 会删除新结构化本地化元数据，操作前必须备份数据库。
...
“不适用”的历史月结不能重新生成月结明细，但历史 `paid`/`reconciled` 记录可以在只读详情单独生成 Word/PDF 并下载，状态不会改变。月结生成后如果补录了同周期订单，待审核或已驳回月结会显示“源数据已发生变化”；管理员填写刷新原因后可以刷新明细，系统会同时更新原批次金额并记录审计。已通过、已结清、历史已支付、历史已对账月结不能直接刷新。未关联批次的历史月结请从“历史月结归档”入口查询，历史明细使用月结快照，已删除订单会显示为已归档订单。
迁移后还要抽查待审核、已驳回、已通过、已结清、零订单和历史导入记录，确认明细数量与实际明细一致。
完整步骤见[完整运维手册](operations-manual.md)第 3.3.1 节。

PR1 还有一项单独的客户/订单模型迁移。它会删除直销来源和渠道字段，并要求客户、订单都有代理商，
...
登录后打开“配置中心 → 系统测试 → 时间模拟”：

1. 设置日期和时间，或点击 `+1 天`、`+7 天`、`+30 天`、`+1 个月`。
2. 需要立刻检查业务结果时，勾选要执行的检查，再点击“设置并立即执行”。
...
本次把机构 Excel 表单从 v1 升级为 v2。机构只填写客户姓名、消费日期、项目、数量、金额和业务备注；
客户编号、客户 ID、机构和表单编号仍放在隐藏的 `__GN_META` 工作表里。下载模板时系统会预填客户姓名
和实际到院日期，上传时会检查客户确实已到院，并且 Excel 消费日期没有被改成别的日期。

这次没有新增 migration、依赖或环境变量，只完成本机测试。发布到 UAT 前不能继续使用旧 v1 模板；要让
...
Dashboard 的“本月各机构销售额”按业务时钟当前自然月显示横向金额条和占比，且复用机构月度销售的正式订单口径；条目可以进入该月已有机构月度销售总览或机构详情。超级管理员可看到当月零营收的活动机构，客服和 BD 仍只能看到原有报表权限范围内的数据。

客服提交客户负责人移交申请后，系统按客户当前业务归属通知对应 BD。通知会写入 BD 的站内提醒；若 BD 绑定了 DingTalk `user_id` 或手机号，还会排队发送钉钉。BD 驳回会通知原客服，批准、直接移交和批量移交完成后会通知原负责人和新负责人，不再给执行审批的 BD 发送动态钉钉提醒。客户详情和机构回传中心的 Excel 回传还必须各自上传沟通截图和结算小票，缺少任一类会被拒绝。通知不广播到其他业务组。
