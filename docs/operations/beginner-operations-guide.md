...
| `develop`（2026-08-07 工作区） | 国际化基础设施、PR-B 日常界面、PR-C 页面及 PR-D 深层输出已加入；配置中心/内部用户、结算、提醒、多维查询和导入支持 `zh_CN`/`ko_KR` | 尚未发布；代理商、客户、配置、审计和提醒的固定文案、默认系统名称与业务异常已接入 Locale；月结、汇率报价和报表导出失败使用结构化消息，提醒模板/实例可按当前语言显示，看板缓存不再绑定某一种语言；发布前备份数据库并确认 `2026_08_07_000100_add_preferred_locale_to_users_table` 至 `2026_08_07_000500_add_localized_content_to_reminders` 五个 migration 全部执行，未知历史自由文本会保留或安全降级，韩文入口已开放 |
...
上一轮月结修复本身没有新增 migration、依赖或服务器命令；本轮韩文化收尾新增 `000400`、`000500` migration，并把 `league/commonmark` 安全升级到锁文件允许的修复版本。部署 UAT 前仍需按完整手册执行门禁、数据库备份和 migration，并人工复验失败详情、报告下载、系统提醒和韩文提示行为。
