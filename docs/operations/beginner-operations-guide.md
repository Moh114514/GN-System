...
| `develop`（2026-08-07 工作区） | 国际化基础设施、PR-B 日常界面、PR-C 页面及 PR-D 第一批深层输出已加入；配置中心/内部用户、结算、提醒、多维查询和导入支持 `zh_CN`/`ko_KR` | 尚未发布；查询/看板/结算文档与失败报告、导出文件名、导入问题报告固定标签、解析任务和通知任务已接入 Locale；发布前备份数据库并确认 `2026_08_07_000100_add_preferred_locale_to_users_table` migration，业务异常码、审计描述和动态错误正文仍待后续阶段，韩文入口暂不开放 |
...
国际化 PR-A、PR-B、PR-C 及 PR-D 第一批当前只在 `develop` 本机工作区完成，不能据此认为 UAT 或 Production 已支持韩文。
发布时要先备份数据库，再按 RC 流程执行 migration；既有用户默认保持 `zh_CN`。业务页面、导出
业务异常码、审计描述和动态错误正文的韩文化属于后续阶段。
