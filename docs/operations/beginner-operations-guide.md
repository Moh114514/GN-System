...
| `develop`（2026-08-07 工作区） | 国际化基础设施、PR-B 日常界面、PR-C 页面及 PR-D 深层输出已加入；配置中心/内部用户、结算、提醒、多维查询和导入支持 `zh_CN`/`ko_KR` | 尚未发布；查询/看板/结算文档与失败报告、导出文件名、导入问题报告固定标签、解析任务和通知任务已接入 Locale；结算/订单/配置中心/内部用户管理业务异常码、结算/订单审计动作描述、导入问题安全报告及历史固定审计描述映射已接入 Locale；发布前备份数据库并确认 `2026_08_07_000100_add_preferred_locale_to_users_table`、`2026_08_07_000200_add_structured_messages_to_import_issues_table`、`2026_08_07_000300_add_structured_failure_to_import_batches_table` migration，未知历史自由文本会保留并标记，导入批次失败原因已结构化存储并兼容旧原文，已知动态错误已保存消息键与命名参数，韩文入口已开放 |
...
主要业务异常码、已知审计动作、历史固定审计描述和导入错误报告安全文案的韩文化已完成；未知历史自由文本会保留原文并标记，导入批次失败原因已结构化存储并兼容旧原文，已知动态错误已保存消息键与命名参数。语言设置页可直接选择 `한국어`；发布到 UAT/Production 前仍需按 RC 流程执行 migration 和入口抽查。
