...
主要业务异常码、已知审计动作、历史固定审计描述和导入错误报告安全文案的韩文化已完成；未知历史自由文本会保留原文并标记，导入批次原始失败原因的数据层回填和未知动态错误精细化参数属于后续阶段。
