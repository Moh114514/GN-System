...
>
> 当前情况：UAT 已运行；Production 只完成目录初始化，尚未部署
...
“无法确认”的月结不能直接审核，必须先核对原始数据并留下审计记录。迁移后还要抽查待审核、已驳回、已通过、已结清、零订单和历史导入记录，确认明细数量与实际明细一致。
...
目标是把 <明确标签，例如 v0.5.0-rc.7> 部署到 <UAT 或 Production>。
