...
国际化 PR-A、PR-B、PR-C 及 PR-D 当前两批只在 `develop` 本机工作区完成，不能据此认为 UAT 或 Production 已支持韩文。
发布时要先备份数据库，再按 RC 流程执行 migration；既有用户默认保持 `zh_CN`。业务页面、导出
主要业务异常码和审计动作的韩文化已完成；导入/审计动态错误正文及安全展示边界属于后续阶段。
