...
当前系统自动生成的预约提醒只有一条：预计到院前一天 18:00 联系客户，并在提醒内容中明确展示预计到院时间；不再生成预约前三天或到院当天的重复任务。若前一天 18:00 已过但预计到院仍在未来，则立即生成一条幂等补偿提醒。预约改期会取消旧的未发送实例并生成新实例，已经发送的实例保留为历史记录；到院或完成时关闭业务待办，但不把已发送通知的 `sent` 历史改写为 `cancelled`。施术结束提醒仍只有术后 7 天和 30 天。提醒实例的幂等键包含客户、施术结束时间和提醒类型，回退并重新设置相同结束时间不会重复生成。旧生命周期自动提醒实例、旧系统规则和模板由客户生命周期清理迁移一次性删除。
