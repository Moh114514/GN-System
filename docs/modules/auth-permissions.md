...
有效期；未归属用户和代理商必须在发布前处理。已启用但尚未接受邀请的 BD/客服可以提前
配置成员历史，但在接受邀请前 `AccessContext` 保持 deny-all，不得执行客户、提醒或转派等
业务操作。

配置中心的成员候选包含所有启用的 BD/客服，并显示其业务日期下的当前业务组，允许管理员
提前配置不重叠的未来转组关系。成员和代理商归属历史提供“结束归属”操作，结束日期和原因
会通过现有 Application Contract 写入并审计；所有默认日期、当前归属和未归属检查统一使用
`BusinessClock`，正式生产环境因此仍等同于真实当前时间。
