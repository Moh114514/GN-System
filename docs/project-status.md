...
> 最后核验：2026-09-14
> 核验依据：当前 `feature/pr1-order-registration` 工作区、PR1–PR5 与审查修复定向测试、模块边界与本地化检查、完整后端门禁和 Vite 构建
> 当前阶段：Phase 6、订单中心、Phase 5 月结运行关系/历史数据闭环及 PR1–PR7 规划能力继续保持；角色/业务组底座、权限范围、客户负责人移交与状态回退审批、订单事实快照和 BD 季度提成已合入本地 `develop`。当前工作区已完成方案 PR1–PR5 的订单登记、机构销售 PDF、客户工作台、机构营收看板和移交通知能力，并完成审查指出的统计口径、当前月下钻和 Excel 凭证一致性修复，但尚未合入 `develop`、发布或完成 UAT/Production 验收；凭证表 migration、Excel v2、机构销售 PDF、客户工作台、机构营收看板和动态 BD 通知的目标环境执行、备份、抽样核验和人工业务验收仍未完成。

本页只描述仓库中可以验证的状态。未来规划见 `docs/source/`，不能据此页之外的
规划内容推断某项能力已经存在。

## 2026-09-10 PR1 订单登记基础
...
## 2026-09-14 PR4 客户工作台迁移

- 客户管理页顶部新增独立 Livewire `CustomerOverview` 子组件和 `CustomerOverviewService`，集中展示待跟进、今日待办、客户生命周期和最近客户记录；原有筛选、批量移交和分页仍由 `CustomerList` 负责。
...
- Dashboard 机构营收改为复用机构月度销售额的正式订单口径：`status=completed`、`record_status=active`、非空 `occurred_on` 和当前权限范围；机构卡片固定展示业务时钟当前自然月，并只下钻到该月，避免季度/年度区间伪装成单月明细。
- 客户详情和机构回传中心的 Excel 回传均要求沟通截图与结算小票各至少一份，并通过 `CompletedOrderRegistrar` 的 `requireEvidence` 统一强制校验；审批完成通知只发送给原负责人和新负责人，不向执行审批的 BD 发送动态钉钉提醒；Dashboard 底部无对应图表的客户状态图例已移除。
- 本轮不新增 migration、依赖或环境变量；已补充订单回传、机构销售、Dashboard 和客户移交通知回归测试。仍未合入 `develop`、部署或完成 UAT/Production 人工验收。
...
| 机构营收看板与 BD 移交通知（方案 PR5） | Dashboard 按机构月度销售正式口径展示包含零营收活动机构的横向营收对比，并固定下钻当前自然月；客户移交申请按业务归属通知 BD，审批结果通知相关客服，动态用户通知同时支持站内和钉钉；Excel 回传与手工登记统一要求两类凭证；不新增 migration | 2026-09-14 |
