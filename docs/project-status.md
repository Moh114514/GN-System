...
| 结算文档与等级建议 | 已从同一内容快照生成私有 Word/PDF/ZIP；PDF 使用镜像内经字符集和 glyf 校验的 `GNSystemCJK.ttf`，覆盖中韩文及 `₩`；等级建议仅经管理员复核后安排下月生效 | 2026-08-07 |
...
- Order 报表读取器在数据库内按 `institution_id` 聚合，固定使用 `occurred_on` 自然月、
  ...
  页面与 XLSX 导出复用同一份汇总结果，Excel 金额保持数值单元格。
- 本 PR 不新增 migration、依赖、汇总表、预聚合或索引；真实量级 UAT 仍需执行查询计划检查。
  当前仅完成本地定向测试，未合入 `develop`、部署或在 UAT/Production 人工验收。
