<div>
    ...
    <form wire:submit="save" class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <fieldset class="mt-6"><legend class="font-semibold">{{ __('config.notification_recipients.dingtalk_heading') }}</legend><div class="mt-3 grid gap-2 sm:grid-cols-2">@foreach ($users as $user)<label class="flex items-center gap-2 {{ $user->dingtalk_user_id ? '' : 'cursor-not-allowed opacity-60' }}"><input type="checkbox" wire:model="dingtalkUserIds" value="{{ $user->id }}" @disabled(blank($user->dingtalk_user_id))><span>{{ $user->name }} ({{ $user->dingtalk_user_id ? __('config.notification_recipients.dingtalk_bound') : __('config.notification_recipients.dingtalk_missing') }})</span></label>@endforeach</div>@error('dingtalkUserIds')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</fieldset>
