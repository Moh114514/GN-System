<div>
    ...
    <section class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <p class="crm-eyebrow">客户详情 · {{ $customer['code'] }}</p>
            <h2>{{ $customer['name'] }}</h2>
            <p>{{ $customer['project_intention'] }} · 建档于 {{ $customer['created_at'] }}</p>
        </div>
        <flux:button class="shrink-0 self-start" :href="route('customers.edit', $customer['id'])" size="sm" icon="pencil-square" wire:navigate>编辑档案</flux:button>
    </section>
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div class="space-y-6">
            <section class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                <div class="flex items-center justify-between"><h3 class="text-lg font-semibold">客户档案</h3><span class="crm-pill tone-blue">{{ $customer['current_status'] }}</span></div>
                ...
                <div class="mt-6 space-y-5">
                    @forelse ($timeline as $event)
                        <article class="relative border-l-2 border-teal-200 pl-5" wire:key="{{ $event['type'] }}-{{ $loop->index }}-{{ $event['occurred_at'] }}">
                            ...
                            <p class="mt-1 text-xs text-zinc-400">{{ $event['institution'] ?? '' }} {{ $event['owner'] ? '· '.$event['owner'] : '' }}</p>
                        </article>
                    @empty
