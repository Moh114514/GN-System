        ...
        <div class="space-y-6">
            <section class="crm-card">
                ...
                <dl class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div><dt class="text-xs text-zinc-500">{{ __('orders.fields.channel') }}</dt><dd class="mt-1 font-medium">{{ $order['channel'] === 'agent' ? __('orders.channels.agent').' · '.($order['agent']['name'] ?? __('orders.values.unknown_agent')) : __('orders.channels.direct').' · '.($order['direct_source']['name'] ?? __('orders.values.unknown_source')) }}</dd></div>
