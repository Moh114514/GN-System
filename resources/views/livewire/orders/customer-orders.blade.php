    ...
    <div class="grid gap-6 xl:grid-cols-[24rem_minmax(0,1fr)]">
        <section class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <form wire:submit="save" class="mt-4 space-y-4">
                ...
                <flux:select wire:model.live="channel" :label="__('orders.fields.channel')" required>
                ...
                @else
                    <flux:select wire:model="directSalesSourceId" :label="__('orders.fields.direct_source')" required>
                        <flux:select.option value="">{{ __('orders.fields.select') }}</flux:select.option>
        ...
        <section class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="crm-table-wrap mt-4"><table class="crm-table">
                <thead><tr><th>{{ __('orders.fields.order') }}</th><th>{{ __('orders.fields.channel') }}</th><th>{{ __('orders.fields.status_label') }}</th><th>{{ __('orders.fields.promotion_fee') }}</th><th></th></tr></thead>
                <tbody>
                @forelse ($context['orders'] as $order)
                    <tr wire:key="order-{{ $order['id'] }}">
                        ...
                        <td>{{ $order['channel'] === 'agent' ? __('orders.channels.agent') : __('orders.channels.direct') }}</td>
                ...
                @empty
                    <tr><td colspan="5" class="py-10 text-center text-zinc-500">{{ __('orders.customer.no_orders') }}</td></tr>
                @endforelse
                </tbody>
            </table></div>
        </section>
